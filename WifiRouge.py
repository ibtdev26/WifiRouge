...
def show_networks(networks):
    ...
    header_fmt = "{:<3} {:<20} {:<5} {:<20} {:>5}"
    row_fmt = "{:<3} {:<20} {:<5} {:<20} {:>5}"
    print(Fore.CYAN + f"\n[+] Found {len(networks)} network(s):\n" + Style.RESET_ALL)
    print(Fore.CYAN + header_fmt.format("ID", "SSID", "CH", "BSSID", "RSSI") + Style.RESET_ALL)
    print(Fore.CYAN + "-" * 70 + Style.RESET_ALL)

    for i, net in enumerate(networks):
        color = get_color(net['rssi'])
        print(color + row_fmt.format(i, net['ssid'], net['channel'], net['bssid'], net['rssi']) + Style.RESET_ALL)
...
def attack_prompt(networks):
    ...
    try:
        selection = input("Enter Network IDs to Attack (comma separated, 'all' for All Networks, or 'exit' to Return): ").strip().lower()
